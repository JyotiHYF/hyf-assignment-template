document.addEventListener("DOMContentLoaded", () => {
  ...
  const taggedMovies = movies.map((m) => {
    const tag = m.rating >= 7 ? "Good" : m.rating >= 4 ? "Average" : "Bad";
    return { ...m, tag };
  });
  ...
  const showCards = (arr) => {
    resultsDiv.innerHTML = arr
      .map(
        ...
        <div class="${m.tag.toLowerCase()}">${m.tag}</div>
      </div>`,
      )
      .join("");
  };
  ...
  const showText = (text) => {
    resultsDiv.innerHTML = `<div class="card">${text}</div>`;
  };

  // 1. Short Titles
  document.getElementById("shortBtn").addEventListener("click", () => {
    const result = taggedMovies.filter((m) => m.title.length < SHORT_LENGTH);
    showCards(result);
  });
